...
var projectSettingLoad = function(){
	...
	$('.jqueryFileTree a').addClass('mask')
		$('.file > a[rel$="' + "setting.json" +'"]').css({'display':'block'})
						...
						var treeReload = function(){
							...
							appConf.readConf(function(jsonConf){
								...
								$('.jqueryFileTree a').addClass('mask')
									...
									$('.file > a[rel$="AsazukeConf-sample.jp.php"]').css({'display':'none'})
		...
		global.Load = {
			...
			sitemapCSV:function(){
				...
				appConf.readConf(function(jsonConf){
					...
					.sort(function(a, b) { return b.time - a.time; }) // DESC
					...
					$('.js_exported_sitemap a').parent().css({'display':'none'});
					//$('.js_exported_sitemap a[rel*="'+ jsonConf.select_project+'"]').parent().css({'display':'block'});
					$('.js_exported_sitemap a[rel*="'+ global.confJson.projectName +'"]').parent().css({'display':'block'});
				});
			},
					...
					case 4:
						...
						$("#div_C .layer-panel").eq(n).load("database.html", function (htmlData, loadStatus){
							...
							appConf.readConf(function(jsonConf){
								...
								$('.jqueryFileTree a').addClass('mask')
									$('.file > a[rel*="asazuke.sqlite"]').css({'display':'block', 'color':'#BFBFBF'});
								$('.file > a[rel$="asazuke.sqlite"]').css({'display':'block', 'color':'#333'});
